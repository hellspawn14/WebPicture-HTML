...
function getElementInnerText(cell) {
    var cellType = cell.get('type');
    cellType = cellType.split('.')[1].toLowerCase();
    if (cellType == 'member' || cellType == 'imageshape' || cellType == 'polygon' || cellType == 'icnshape') {
...
paper.on('cell:pointerdown', function (cell, evt, x, y) {
    ...
    if (cell.model instanceof joint.dia.Link) {} else {

        if (!cell.model.get('embeds') || cell.model.get('embeds').length === 0) {
        ...
        if (cell.model.get('parent')) {
            graph.getCell(cell.model.get('parent')).unembed(cell.model);
        }
...
function validateDiagram() {
    ...
    if (links == false) {
        errorIndex++;
        errorReport += errorIndex + " - There are unconnected links in diagram " + '<br>';
    }
 ...
 */
function help() {
    bootbox.alert("Drag and drop elements from the Palette panels to the main canvas", function () {
        //Hacer nada 
    });
}
